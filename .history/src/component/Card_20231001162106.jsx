...
function Card({ link, icon, title, onClick }) {
  ...
  return (
    <div
      className="max-w-sm  mx-1 my-6 cursor-pointer shadow-lg rounded-lg overflow-hidden hover:shadow-xl transition duration-300 transform hover:scale-105"
      onClick={onClick}
    >
      <div className="bg-sky-800 rounded-lg overflow-hidden shadow-lg flex flex-col items-center justify-center">
        <Link to={link} className="text-blue-600 hover:text-blue-800">
          ...
          <FontAwesomeIcon
            icon={icon}
            size="2xs"
            className=" w-auto p-10 h-auto object-cover"
            style={{color: "#bbc9e2",}}
          />
