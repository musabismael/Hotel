...
const HousekeepingMaintenance = () => {
  const [housekeepingTasks, setHousekeepingTasks] = useState([
    { id: 1, task: 'Clean Room 101', status: 'Pending', details: 'Clean room, change sheets, vacuum.' },
    { id: 2, task: 'Replace Towels in Room 102', status: 'Completed', details: 'Replace towels and toiletries.' },
    { id: 3, task: 'Clean Lobby Area', status: 'Pending', details: 'Sweep, mop, and dust lobby.' },
  ]);

  const [maintenanceRequests, setMaintenanceRequests] = useState([
    { id: 1, request: 'Fix Broken Sink in Room 103', status: 'Pending', details: 'Sink is leaking.' },
    { id: 2, request: 'Replace Light Bulbs in Hallway', status: 'In Progress', details: 'Replace hallway light bulbs.' },
  ]);

  const [newTask, setNewTask] = useState({ task: '', status: 'Pending', details: '' });
  const [newRequest, setNewRequest] = useState({ request: '', status: 'Pending', details: '' });
  ...
  const handleEditRequest = () => {
    if (selectedRequest) {
      const updatedRequests = maintenanceRequests.map((request) =>
        request.id === selectedRequest.id ? { ...request, ...newRequest } : request
      );
      setMaintenanceRequests(updatedRequests);
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="my-4 p-4 border rounded-lg">
        <h2 className="text-lg text-white font-semibold mb-4">Housekeeping and Maintenance</h2>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <h3 className="text-lg text-white font-semibold mb-2">Housekeeping Tasks</h3>
            <div className="mb-2">
              <label htmlFor="filterTaskStatus" className="text-white">Filter by Status:</label>
              <select
                id="filterTaskStatus"
            ...
            <div className="mb-2">
              <label htmlFor="sortTaskStatus" className="text-white">Sort by Status:</label>
              <select
                id="sortTaskStatus"
                className="w-full border rounded-md py-2 px-3"
            ...
            <ul className="space-y-2">
              {sortTasks(housekeepingTasks, sortTaskStatus).map((task) => (
                ...
                >
                  <span className="text-gray-800 font-semibold">{task.task}</span> ({task.status})
                </li>
              ))}
            </ul>
            <div className="mt-4">
              ...
              <button
                ...
                onClick={selectedTask ? handleEditTask : handleAddHousekeepingTask}
              >
          ...
          <div>
            <h3 className="text-lg text-white font-semibold mb-2">Maintenance Requests</h3>
            <div className="mb-2">
              <label htmlFor="filterRequestStatus" className="text-white">Filter by Status:</label>
              <select
            ...
            <ul className="space-y-2">
              {sortRequests(maintenanceRequests, sortRequestStatus).map((request) => (
                <li
                  key={request.id}
                  className={`p-2 rounded-lg ${
                    request.status === 'In Progress' ? 'bg-blue-200' : 'bg-yellow-200'
                  }`}
                  onClick={() => setSelectedRequest(request)}
                >
                  <span className="text-gray-800 font-semibold">{request.request}</span> ({request.status})
                </li>
              ))}
            </ul>
            <div className="mt-4">
              ...
              <button
                className="bg-blue-500 text-white py-2 px-4 rounded-md mt-2"
                onClick={selectedRequest ? handleEditRequest : handleAddMaintenanceRequest}
              >
      ...
      <div className="my-4 p-4 border rounded-lg">
        {selectedTask && (
          <>
            <h3 className="text-lg text-white font-semibold mb-2">Task Details</h3>
            <div className="mb-2">
        ...
        {selectedRequest && (
          <>
            <h3 className="text-lg text-white font-semibold mb-2">Request Details</h3>
            <div className="mb-2">
            ...
            <div className="mt-4">
              <button
                className="bg-blue-500 text-white py-1 px-2 rounded-md mr-2"
                onClick={() => handleRequestStatusChange(selectedRequest.id, 'In Progress')}
              >
                Start Work
              </button>
              <button
